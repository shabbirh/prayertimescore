...
using static prayertimescore.PrayerTimes.Library.Calender.Calendar;
...
namespace prayertimescore.PrayerTimes.Library.Calender
{
    public sealed class SolarDate
    {
        ...
        public int DaysPast
        {
            get
            {
                var date = ConvertToGregorian(ArrayType[0], 1, 1, DateType.Persian);
        ...
        private readonly string[] _solarMonthsEnglish = { "Farvardeen", "Ordebesht", "Khordad", "Tir", "Mordad", "Shahrivar", "Mehr", "Aban", "Azar", "Dey", "Bahman", "Esfand" };

        private readonly string[] _days = {"یکم","دوم","سوم","چهارم","پنجم","ششم","هفتم","هشتم","نهم","دهم","یازدهم","دوازدهم","سیزدهم",
            ...
            else
            {
                switch (dateTimeFormatSpecifier)
                {
                    ...
                    case "m":
                        ...
                        break;
                    case "english_month":
                        dateFormat = _solarMonthsEnglish[ArrayType[1] - 1];
                        break;
                    case "english_day":
                        dateFormat = ArrayType[2].ToString();
                        break;
                    case "english_year":
                        dateFormat = ArrayType[0].ToString();
                        break;
                    case "english_formatted":
                        dateFormat = $"{ArrayType[2]} {_solarMonthsEnglish[ArrayType[1] - 1]} {ArrayType[0]}";
                        break;
                    case "N":
