...
using System.Linq;
using prayertimescore.PrayerTimes.Library.Enumerations;
...
namespace prayertimescore.PrayerTimes.Library.Calender
{
    public static class Calendar
    {
        ...
        {
            return ConvertToPersian(null, year, month, day, dateType == DateType.Gregorian);
        }
        ...
        private static SolarDate ConvertToPersian(DateTime? date, int year, int month, int day, bool isDateTime)
        {
            ...
            if (isDateTime)
            {
                if (date.HasValue)
                {
                    jd = GregorianToJd(date.Value.Year, date.Value.Month, date.Value.Day);
        ...
        {
            var jd = !isSlamic ? PersianToJd(year, month, day) : IslamicToJd(year, month, day);
            var miladi = JdToGregorian(jd);
            var date = new DateTime(miladi[0], miladi[1], miladi[2], hour, minute, second);
            return date;
        }

        private static float GregorianToJd(int year, int month, int day) => (float)(GregorianEpoch - 1 +
            (365 * (year - 1)) +
        ...
        private static int[] JdToPersian(float jd)
        {
            ...
            var day = (int)(jd - PersianToJd(year, month, 1)) + 1;
            var res = new[] { year, month, day };
            return res;
        }

        private static float PersianToJd(int year, int month, int day)
        {
        ...
        private static int[] JdToGregorian(float jd)
        {
            ...
            float leapadj = wjd < GregorianToJd(year, 3, 1) ? 0 : LeapGregorian(year) ? 1 : 2;
            var month = (int)Math.Floor((((yearday + leapadj) * 12) + 373) / 367);
            var day = (int)(wjd - GregorianToJd(year, month, 1) + 1);
        ...
        private static int[] JdToIslamic(float jd)
        {
            jd = (float)(Math.Floor(jd) + 0.5);
            var year = (int)Math.Floor(((30 * (jd - IslamicEpoch)) + 10646) / 10631);
            var month = (int)Math.Min(12, Math.Ceiling((jd - (29 + IslamicToJd(year, 1, 1))) / 29.5) + 1);
            var day = (int)(jd - IslamicToJd(year, month, 1) + 1);
            var res = new[] { year, month, day };
            return res;
        }
    }
